...
def fastly_purge(*args):
    ...
    for govuk_path in args:
        for hostname in hostnames_to_purge:
          run("curl -s -X PURGE -H 'Host: {0}' -H 'Fastly-Soft-Purge: 1' {1}{2}".format(hostname, govuk_fastly, govuk_path.strip()))

@task
@runs_once
@roles('class-cache')
def fastly_force_purge(*args):
    "Forcibly purge items from Fastly, eg \"/one,/two,/three\""
    govuk_fastly = 'http://www-gov-uk.map.fastly.net'
    hostnames_to_purge = ['www.gov.uk', 'assets.digital.cabinet-office.gov.uk']
    for govuk_path in args:
      for hostname in hostnames_to_purge:
